        ...
        if (newState.channel) {
            ...
            if (
                ...
                newState.channel.permissionOverwrites
                    ...
                    ?.allow.has(PermissionFlagsBits.PrioritySpeaker) // Bot has priority speaker in the channel so it is a lobby
            ) {
                const channel = newState.guild.channels.cache.find(
                    (c) =>
                        c.type === ChannelType.GuildVoice &&
                        newState.member && c.permissionOverwrites.resolve(newState.member.id) &&
                        c.permissionOverwrites
                            .resolve(newState.member.id)
                            ?.allow.has(PermissionFlagsBits.AddReactions) &&
