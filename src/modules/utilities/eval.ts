...
export default class Eval extends GargoyleModule {
    ...
    public override async executeTextCommand(client: GargoyleClient, message: Message) {
        client.logger.trace(
            `Eval command executed by ${message.author.tag} (${message.author.id}) in ${message.guild?.name ?? 'DM'} (${message.guild?.id ?? 'DM'})`
        );
        ...
        try {
            ...
            const fn = new AsyncFunction('client', 'message', `return (${code})`);
            const result = await fn(client, message);
